...
async function subsetFonts(
  ...
  if (!subsetPerPage) {
    ...
    const globalFontUsageTextMap = {};
    for (const fontUsage of fontUsagesConcat) {
      if (globalFontUsageTextMap[fontUsage.fontUrl]) {
        globalFontUsageTextMap[fontUsage.fontUrl] = [
          ...new Set([
            ...globalFontUsageTextMap[fontUsage.fontUrl],
            ...fontUsage.text,
          ]),
        ...
        globalFontUsageTextMap[fontUsage.fontUrl] = fontUsage.text;
      }
    }
    ...
    for (const htmlAssetTextWithProps of htmlAssetTextsWithProps) {
      for (const fontUsage of htmlAssetTextWithProps.fontUsages) {
        fontUsage.text = globalFontUsageTextMap[fontUsage.fontUrl];
      }
    }
  }
